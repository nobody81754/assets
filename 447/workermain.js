...
async function LoadScripts(scriptsArr) {
    if (scriptsArr.length === 1) {
        ...
        url = "https://cdn.jsdelivr.net/gh/gn-math/assets@main/447/"+((url+"").replace(location.href.replace(location.href.split("/").pop(), ""), ""));
        await import(url)
    } else {
        const scriptStr = scriptsArr.map(url => `import "${"https://cdn.jsdelivr.net/gh/gn-math/assets@main/447/"+((url+"").replace(location.href.replace(location.href.split("/").pop(), ""), ""))}";`).join("\n");
